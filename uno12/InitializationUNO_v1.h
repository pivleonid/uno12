 ...
 *
 * int uno_open_normal(uint8_t uno_index) -> Это функция настройки синтезатора. В этой функции происходит включение
 ...
 *
 * int normal_freq() -> функция устанавливает частоту повышенной точности до 1 Гц. 
 *
 ...
 * 
 * int uno_open_fast( uint8_t uno_index) -> Настройка синтезатора в быстром режиме. Поддерживаются 8 профилей частоты.
 * 
 ...
 * 
 * Для переключения между двумя возможными режимами работы синтезатора необходимо придерживаться следующего алгоритма:
 * 1) Выключить выбранный синтезатор- uno_close
 * 2) Включить синтезатор в нужном режиме uno_open_normal или uno_open_fast
 * 
 * 
 ...
 *
 * При включении синтезатора лучше всего проверять бит корректного включения:
 * while (uno_open(0) != 0)
 *		{
 *			uno_close(0);
 *			uno_open(0);
...
/*Эти функции написаны по просьбе Петра- для ускорения работы. */
